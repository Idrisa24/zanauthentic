    ...
    <div class="py-12">
      <section class="px-4 sm:px-6 lg:px-4 xl:px-6 pt-4 pb-4 sm:pb-6 lg:pb-4 xl:pb-6 space-y-4">
        <header class="flex items-center justify-between">
          <h2 class="text-lg leading-6 font-medium text-black">Slides</h2>
        ...
        <ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4">
          <li v-for="slide in slides" :key="slide.id">
            <a :href="route('slides.show', {slide: slide.id})" class="hover:bg-light-blue-500 hover:border-transparent hover:shadow-lg group block rounded-lg p-4 border border-gray-200">
              <dl class="grid sm:block lg:grid xl:block grid-cols-2 grid-rows-2 items-center">
                ...
                <div>
                  <dt class="sr-only">Slide Position</dt>
                  <dd class="group-hover:text-light-blue-200 text-sm font-medium sm:mb-4 lg:mb-0 xl:mb-4">
                    {{slide.slide_position}}
                  </dd>
                </div>
                <div class="col-start-2 row-start-1 row-end-3">
                  <dt class="sr-only">slide Image</dt>
                  <dd class="flex justify-end sm:justify-start lg:justify-end xl:justify-start -space-x-2">
                    ...
                    <img :src="'/storage/'+slide.slide_photo_path" :alt="'slide.slide_title'"  class="w-7 h-7 rounded bg-gray-100 border-2 border-white" />
                  </dd>
                </div>
              </dl>
            </a>
          ...
          <li class="hover:shadow-lg flex rounded-lg">
            <a :href="route('slides.create')" class="hover:border-transparent hover:shadow-xs w-full flex items-center justify-center rounded-lg border-2 border-dashed border-gray-200 text-sm font-medium py-4">
              New Slide 
            </a>
          </li>
        </ul>
